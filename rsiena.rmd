...
<!---to build website, I have set, eval=FALSE for now---> 
...
#construct two separate datasets for each period. 
deps <- mydata$depvars
...
We tested for time heterogeneity in outdegree. Model fit did not improve. 
...
Check if model with gwespFF fits better. No similar fit.
...
asmatkable <- kbl(res, booktabs=TRUE, digits=3, caption="Table 4: RSiena results for hypothesis 1", align = "c") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
...
xtable.sienaFit(ansM10c, file="ansM10c.html", type="html", digits=3)
```
...
load("ansM8c.RData")
ansM8c
tt <- sienaTimeTest(ansM8c)
tt
ttb <- sienaTimeTest(ansM8c,  effects=c(11,24, 41))
ttb
#str(tt)
ttb$JointTest
ttb$EffectTest
...
```{r}
# this does not work!! myeff_m4 <- ansM12$effects
myeff_m5 <- myeff_m4

myeff_m5 <- includeTimeDummy(myeff_m5, sameX, interaction1 = "partij", name="fnet", timeDummy="2")
myeff_m5 <- includeTimeDummy(myeff_m5, sameX, interaction1 = "partij", name="atmnet", timeDummy="2")
myeff_m5 <- includeTimeDummy(myeff_m5, sameX, interaction1 = "partij", name="rtnet", timeDummy="2")
myeff_m5

myalgorithm <- sienaAlgorithmCreate( projname = 'M13')
ansM13 <- siena07( myalgorithm, data = mydata, prevAns=ansM12, effects = myeff_m5, useCluster=TRUE, nbrNodes=10, initC=TRUE, batch=TRUE)
ansM13 
save(ansM13, file=paste("ansM13",".RData",sep=""))
load("ansM13.RData")
ansM13
sienaTimeFix(effects, data)
...
myalgorithm <- sienaAlgorithmCreate( projname = 'M12_t12')

ansM12_t12 <- siena07( myalgorithm, data = mydata12, effects = myeff, prevAns = ansM12, useCluster=TRUE, nbrNodes=10, initC=TRUE, batch=TRUE)
ansM12_t12 
save(ansM12_t12, file=paste("ansM12_t12",".RData",sep=""))

ansM12_t12b <- siena07( myalgorithm, data = mydata12, effects = myeff, prevAns = ansM12_t12, useCluster=TRUE, nbrNodes=10, initC=TRUE, batch=TRUE)
ansM12_t12b 
save(ansM12_t12b, file=paste("ansM12_t12b",".RData",sep=""))

# 
# *** Standard errors not reliable ***
# The following is approximately a linear combination 
# for which the data carries no information:
# -1 * beta[3] + -0.45 * beta[6] + 0.45 * beta[7] + -0.45 * beta[8]
# It is advisable to drop one or more of these effects.


ansM12_t12c <- siena07( myalgorithm, data = mydata12, effects = myeff, prevAns = ansM12_t12b, useCluster=TRUE, nbrNodes=10, initC=TRUE, batch=TRUE)
ansM12_t12c 
save(ansM12_t12c, file=paste("ansM12_t12c",".RData",sep=""))

ansM12_t21 <- siena07( myalgorithm, data = mydata12, effects = myeff, prevAns = ansM12, useCluster=TRUE, nbrNodes=10, initC=TRUE, batch=TRUE)
ansM12_t21 
save(ansM12_t12, file=paste("ansM12_t21",".RData",sep=""))

ansM12_t21b <- siena07( myalgorithm, data = mydata12, effects = myeff, prevAns = ansM12_t21, useCluster=TRUE, nbrNodes=10, initC=TRUE, batch=TRUE)
ansM12_t21b 
save(ansM12_t21b, file=paste("ansM12_t21b",".RData",sep=""))

ansM12_t21c <- siena07( myalgorithm, data = mydata12, effects = myeff, prevAns = ansM12_t21b, useCluster=TRUE, nbrNodes=10, initC=TRUE, batch=TRUE)
ansM12_t21c 
save(ansM12_t21c, file=paste("ansM12_t21c",".RData",sep=""))
