...
if (!require("netseg", character.only = TRUE)) {install.packages("netseg", dependencies=TRUE)}
...
```{r}
colmat2 <- matrix(NA, nrow=13, ncol=9)
colmat2[,1] <- netseg::coleman(gfnet1, vattr=partij_int)
colmat2[,2] <- netseg::coleman(gfnet2, vattr=partij_int)
colmat2[,3] <- netseg::coleman(gfnet3, vattr=partij_int)
colmat2[,4] <- netseg::coleman(grtnet1, vattr=partij_int)
colmat2[,5] <- netseg::coleman(grtnet2, vattr=partij_int)
colmat2[,6] <- netseg::coleman(grtnet3, vattr=partij_int)
colmat2[,7] <- netseg::coleman(gatmnet1, vattr=partij_int)
colmat2[,8] <- netseg::coleman(gatmnet2, vattr=partij_int)
colmat2[,9] <- netseg::coleman(gatmnet3, vattr=partij_int)

colnames(colmat2) <- c("following T1", "following T2", "following T3","atmentions T1","atmentions T2","atmentions T3","retweets T1","retweets T2","retweets T3") 

rownames(colmat2) <- levels(keyf$Partij)[-c(1,7,9,15,18,19)]

rownames(colmat2) <- c("CDA (19)", "CU (5)", "DENK (3)", "D66 (19)", "FvD (2)", "GroenLinks (14)", "PvdA (9)" ,      "PvdDieren (5)", "PVV (18)" , "SGP (3)", "SP (14)",  "VVD (32)", "50Plus (4)") 

colmat2 <- colmat2[c(12,1,4,9,11,6,7,2,8,13,3,10,5),]


# #why NaN e.g. FvD atmentions t2
# V(gatmnet2)$label <- partij_int
# V(grtnet2)$label <- partij_int
# 
# gatmnet2_subset <- subgraph.edges(graph=gatmnet2, eids=which(partij_int==6), delete.vertices = TRUE)
# plot(gatmnet2_subset)
# 
# gatmnet2_subset <- subgraph(graph=gatmnet2, vids=which(V(gatmnet2)$label==6))
# plot(gatmnet2_subset)
# #because no out ties!. 
# 
# #why -1 e.g. FvD rtweets t2
# grtnet2_subset <- subgraph.edges(graph=grtnet2, eids=which(partij_int==6), delete.vertices = TRUE)
# plot(grtnet2_subset)
# grtnet2_subset <- subgraph(graph=grtnet2, vids=which(partij_int==6))
# plot(gatmnet2_subset)
# #because no ingroup tie. 



colmat2kable <- kbl(colmat2, booktabs=TRUE, digits=2, caption="RR.Coleman’s Homophily Index within the three Twitter layers among Dutch MPs (2017), per political party (party size between brackets)", align = "c") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  column_spec(column = 1, width = "3in")
